...
func _draw() -> void:
    ...
    draw_string(schrift, Vector2(22.0, 46.0), Zahl.kurz(Spielstand.credits) + " ¢",
        HORIZONTAL_ALIGNMENT_LEFT, -1, 34, Color(0.95, 0.97, 1.0))

    draw_string(schrift, Vector2(24.0, 76.0), "+" + Zahl.kurz(Spielstand.rate()) + " ¢/s",
    ...
    if Spielstand.protokolle > 0:
        draw_string(schrift, Vector2(breite - 202.0, 46.0),
            "%d Protokolle" % Spielstand.protokolle,
